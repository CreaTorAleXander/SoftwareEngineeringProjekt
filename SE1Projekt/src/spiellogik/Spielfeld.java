...
public class Spielfeld {
	...
	void generiereHindernisse() {
		...
		int anzahlGrosseH = 0; // Anz. der bereits gesetzten gro�en Hindernisse

		// 5 Hindernisse, die zufaellig 1 oder 2 Zeilen auseinander liegen (Spielfeld 10
		// Felder gro�, 5 Hindernise => max. 2 Zeilen Abstand)
		for (int i = r.nextInt(2); i < 10; i = i + zeile) {

			int spalte = r.nextInt(8) + 1; // Zufaellige Platzierung des Hindernisses in der Spalte
			int hindernisart = r.nextInt(2); // F�r Zufallswahl ob kleines oder gro�es Hindernis platziert werden soll

			if (hindernisart == 1 && anzahlGrosseH >= 2) // Falls keine kleinen HIndernisse mehr �brig sind, soll ein
															// gro�es platziert werden
				hindernisart = 0;

			if (hindernisart == 0 && anzahlKleineH >= 3) // Falls keine gro�en Hindernisse mehr �brig sind, soll ein
			...
			else if (hindernisart == 1 && anzahlGrosseH < 2) { // Platzieren eines gro�en Hindernisses
				if (spalte == 8)
					spalte--;
				spielfeld[i][spalte] = new Hindernis();
	...
	public void bewegeFigur(Koordinate ziel) {
		...
		// Exception-Handling f�r nicht bewegbare Figur?
	}
