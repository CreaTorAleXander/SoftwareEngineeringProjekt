...
public class Spiel {
	...
	/**
	 * Leitet den Spielbeginn ein. Keine weiteren Ma�nahmen f�r den Spielfluss erforderlich.
	 * 
		...
		while (anzahlDerZuege > 0) {
			...
			if (antwort.equals("JA")) {
					...
					try {
						spielfeld.waehleFigur(k1, spieler);
							...
							if(gewaehlteFiguren.contains(spielfeld.gewaehlteFigur)) {
								throw new InputMismatchException("Figur wurde schon gew�hlt!");
							} else {
				...
				do {
					...
					//�berpr�fung, ob der Zug m�glich ist
					if (spielfeld.pruefeBewegung(k1)) {
						isZugMoeglich = true;
						spielfeld.bewegeFigur(k1);
					} else {
						System.err.println("Zug nicht m�glich");
					}
				} while (!isZugMoeglich);
...
//			}
	 ...
	 * @return Koordinatenwert einer Achse
	 */
	public static Koordinate konvertiereEingabe2(String input) {
	...
	/**
	 * Pr�ft auf das Ende des Spiels (Keine Figuren mehr vorhanden)
	 * 
	...
	/**
	 * Fuehrt die Schadensberechnung fuer beide Spieler durch und
	 * zeigt welche Angriffe in dieser Runde stattfanden und welche Figuren in dieser Runde gestorben sind.
	 * 
	 * @param spielerEins Der erste Spieler
	 * @param spielerZwei Der zweite Spieler
	 * @param feld Das Spielfeld
